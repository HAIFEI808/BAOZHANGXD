...
static void CST816D_GPIO_Init(void)
{
    ...
    /* Route PB10 to EXTI10 (SYSCFG EXTICR[2] bits 11:8 = 0x1 for GPIOB) */
    SYSCFG->EXTICR[2] &= ~(0xFUL << 8);
    SYSCFG->EXTICR[2] |=  (0x1UL << 8);
    /* Configure EXTI line 10: unmask, falling edge */
    EXTI->IMR  |= (1UL << 10);
    EXTI->FTSR |= (1UL << 10);
    /* NVIC priority 6 (FreeRTOS safe range: 5-15); IRQ enabled after chip init */
...
void CST816D_EnableTouchIRQ(void)
{
...
/**
 * @brief 带软件消抖的手势读取
 * @param sample_ms 每次采样间隔 (ms)，典型值 20
 * @return 手势值 (CST816D_GESTURE_*)，三次采样不一致则返回 CST816D_GESTURE_NONE
 * @note  连续 3 次读取手势寄存器，仅当 3 次结果完全一致时才确认有效，
 *        消除触摸面板的瞬时抖动和误触发
 */
uint8_t CST816D_GetGesture_Debounced(uint8_t sample_ms)
{
    uint8_t gesture1, gesture2, gesture3;

    gesture1 = CST816D_GetGesture();
    if (gesture1 == CST816D_GESTURE_NONE)
        return CST816D_GESTURE_NONE;

    HAL_Delay(sample_ms);
    gesture2 = CST816D_GetGesture();
    if (gesture2 != gesture1)
        return CST816D_GESTURE_NONE;

    HAL_Delay(sample_ms);
    gesture3 = CST816D_GetGesture();
